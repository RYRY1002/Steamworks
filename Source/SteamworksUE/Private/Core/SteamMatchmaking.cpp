...
{
	uint32 TmpIP = 0;
	USteamworksUEUtils::ConvertIPStringToUint32(IP, TmpIP);
...
bool USteamMatchmaking::GetLobbyGameServer(FSteamID SteamIDLobby, FString& GameServerIP, int32& GameServerPort, FSteamID& SteamIDGameServer) const
{
	...
	GameServerIP = USteamworksUEUtils::ConvertIPToString(TmpIP);
	return bResult;
}
...
{
	uint32 TmpIP = 0;
	USteamworksUEUtils::ConvertIPStringToUint32(GameServerIP, TmpIP);
...
void USteamMatchmaking::OnFavoritesListChanged(FavoritesListChanged_t* pParam)
{
	...
	m_OnFavoritesListChanged.Broadcast(USteamworksUEUtils::ConvertIPToString(pParam->m_nIP), pParam->m_nQueryPort, pParam->m_nConnPort, pParam->m_nAppID, Flags, pParam->m_bAdd, pParam->m_unAccountId);
}
...
{
	m_OnLobbyGameCreated.Broadcast(pParam->m_ulSteamIDLobby, pParam->m_ulSteamIDGameServer, USteamworksUEUtils::ConvertIPToString(pParam->m_unIP), pParam->m_usPort);
}
